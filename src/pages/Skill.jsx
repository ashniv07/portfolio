...
function Skill() {
  const skillImages = [Js, Css, Html, Re, Docker, Sp, Github, Java];
  ...
  return (
    ...
    >
      <div className="flex items-center mt-15  gap-30">
        {/* Title that rolls in every time in view */}
        <motion.div
          initial={{ y: -200, opacity: 0 }}
          animate={isInView ? { y: 0, opacity: 1 } : { y: -200, opacity: 0 }}
          transition={{ type: "spring", stiffness: 60, damping: 12 }}
        >
          <h1 className="text-white font-bold text-9xl tracking-widest -rotate-90 origin-center">
            SKILLS
          </h1>
        </motion.div>

        {/* Glowing skill icons that blink */}
        <div className="grid grid-cols-3 gap-12">
          {skillImages.map((img, index) => (
            <motion.div
              key={index}
              className="relative rounded-2xl w-40 h-40 overflow-hidden border-4 border-transparent animate-glow"
